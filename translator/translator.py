...
class Translator:
    ...
    def operation_with_rax_offset_value(
        self, operator: str, offset: int
    ) -> Instruction:
        if operator == "+":
    ...
    def process_binary_op_var_and_num(
        self, left: Node, right: Node, var_str: str, operator: str
    ) -> None:
    ...
    def process_binary_op_var_and_var(
        self, left: Node, right: Node, var_str: str, operator: str
    ) -> None:
        ...
        operation_to_rax = self.operation_with_rax_offset_value(
            operator, var_right_offset
        )
        mov_rax_to_mem = OffsetInstruction(OPCODE.MOVA, self.variable_offset[var_str])
        self.commands += [mov_to_rax, operation_to_rax, mov_rax_to_mem]

    def process_binary_op_num_and_num(
        self, left: NumberNode, right: NumberNode, var_str: str, operator: str
    ) -> None:
        res: int
        if operator == "+":
    ...
    def calculate_cmp(self, left_node: Node, right_node: Node) -> list[Instruction]:
        ...
        if isinstance(left_node, NumberNode):
            ...
            imov = OffsetInstruction(
                OPCODE.IMOV, self.variable_offset[left_node.var.text]
            )
            commands += [imov]
        if isinstance(right_node, NumberNode):
            ...
            icmp = OffsetInstruction(
                OPCODE.ICMP, self.variable_offset[right_node.var.text]
            )
            commands += [icmp]
        return commands

    def process_while_condition(
        self, prev_len: int, node: BinaryOp
    ) -> list[Instruction]:
    ...
    def process_if_statement(self, node: WhileIfNode) -> None:
        ...
        if node.else_node is not None:
            ...
            commands_len = len(self.commands)
        self.commands = (
            self.process_if_condition(commands_len, condition) + self.commands
        )
    ...
    def process_output(self, node: PrintNode):
        ...
        if type == TokenEnum.STRING:
            ...
            mov_to_buff = AdModRegAdressInstruction(
                OPCODE.MOV, 9, REGISTERS.RAX, self.buffer_pointer
            )
            cmpa = Instruction(OPCODE.CMPA)
            ...
            if node.var_type.text == "int":
                ...
                self.commands.append(
                    OffsetInstruction(
                        OPCODE.IMOV, self.variable_offset[node.get_token_text()]
                    )
                )
                self.commands.append(
                    AdModRegAdressInstruction(
    ...
    def process_input(self, node: ReadNode):
        ...
        self.commands.append(
            AdModRegAdressInstruction(OPCODE.MOV, 1, REGISTERS.RAX, 40000)
        )
        self.commands.append(
            OffsetInstruction(OPCODE.MOVA, self.variable_offset[variable])
        )
...
def main(source, target, debug_file):
    ...
    with open(target, "br+") as fw, open(debug_file, "w") as file_debug:
        ...
        for instruction in instr_list:
            file_debug.write(
                f" {j} - {instruction.get_bytes_value().hex()} - {instruction}\n"
            )
...
if __name__ == "__main__":
    assert (
        len(sys.argv) == 4
    ), "Wrong arguments: translator.py <input_file> <target_file> <debug_file>"
